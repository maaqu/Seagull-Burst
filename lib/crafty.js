	...
	*/
	...
	_enterFrame: function () {
		...
		l = q.length;
                ...
                // If we have Collision, use .hit for more accurate collision
                // detection.
                if(l > 0 && this.__c["Collision"]) {
                  // HACK: We need to check whether there'd be a collision one pixel
                  //       below. Problem: we need to use an existing entity's polygon
                  //       in the hit query and we can't make a clone of our own.
                  //       Therefore, move ourselves temporarily downwards one pixel.
                  //       This may cause problems elsewhere.
                  this.y += 1;

                  var hits = this.hit(this._anti);
                  if(hits) {
                    hit = hits[0];
                  }
		...
		if (hit) { //stop falling if found
		  if (this._falling) { 
                    ...
                    this.trigger('Moved', { x: this.x, y: oldY }); // Note: no Moved if we weren't falling in the first place
                  }
		} else {
	...
	stopFalling: function (e) {
          // We fell into something, lift ourselves on top of it
          if(e) {
            if(this.__c["Collision"]) {
              if(e.type == "MBR")
                this.y = e.obj._y - this._h;
              else if(e.normal.y > 0) {
                this.y += e.overlap;
                console.log(e);
              }
            }
